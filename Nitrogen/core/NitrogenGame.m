...
NSString * const NitrogenGameSaveStatesChangedNotification = @"NitrogenGameSaveStatesChangedNotification";

@implementation NitrogenGame
{
    NSArray     *saveStates;
...
+ (NSArray*)gamesAtPath:(NSString*)gamesPath saveStateDirectoryPath:(NSString*)saveStatePath
{
    ...
    for(NSString *file in files) {
        NitrogenGame *game = [NitrogenGame gameWithPath:[gamesPath stringByAppendingPathComponent:file] saveStateDirectoryPath:saveStatePath];
...
{
    return [[NitrogenGame alloc] initWithPath:path saveStateDirectoryPath:saveStatePath];
}

- (NitrogenGame*)initWithPath:(NSString*)path saveStateDirectoryPath:(NSString*)saveStatePath
{
    NSAssert(path.isAbsolutePath, @"NitrogenGame path must be absolute");
...
{
    [self _loadSaveStates];
    [[NSNotificationCenter defaultCenter] postNotificationName:NitrogenGameSaveStatesChangedNotification object:self userInfo:nil];
}
...
- (NSString*)gameTitle
{
    ...
    if (title == nil) {
        ...
        uint32_t titleOffset = 0x240 + 0x100 * [NitrogenGame preferredLanguage];
...
{
    return [NSString stringWithFormat:@"<NitrogenGame 0x%p: %@>", self, self.path];
}
